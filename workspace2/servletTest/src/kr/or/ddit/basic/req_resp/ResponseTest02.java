...
public class ResponseTest02 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*
		 	-redirect
		 	  1) 다른페이지로 제어가 넘어가게한다.(직접 파라미터를 넘길수 없다.)
		 	  2) 응답시 브라우저에게 '이동할URL'을 전송하면 브라우저가
...
//		response.sendRedirect(request.getContextPath() + "/responseRedirectTest.do");
		...
		String tel = "010-3054-5125";
		
		response.sendRedirect(request.getContextPath() + "/responseRedirectTest.do?username="+
								name+ "&tel=" + tel);
	}
