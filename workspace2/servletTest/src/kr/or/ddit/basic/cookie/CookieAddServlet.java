...
public class CookieAddServlet extends HttpServlet {
	 ...
	 */
		Cookie nameCookie = new Cookie("name", URLEncoder.encode("천아토","utf-8"));
		int age = 25;
	...
	/*
	 	2) 쿠키 속성 설정
	 	  - cookie변수.setPath("적용경로"); ==> 지정한 경로와 그하위경로에 쿠키를 사용할 수 있다.
	 	  								==> 생략하면 쿠키를 설정한 당시의 경로가 설정된다.
	 	  - cookie변수.setMaxAge(유지시간); ==> 단위 : 초
	 	  								==> 유지시간이 -1이면 웹브라우저가 종료될 때 까지 유지된다(기본값)
	 	  								==> 유지시간이 0이면 즉시 삭제된다.
	 	  - cookie변수.setDomain("적용 도메인명");
	 	  								==> 예) ".ddit.or.kr" ==> "www.ddor.or.kr" , "www2.ddit.or.kr", ...
	 	  - cookie변수.setSecure(보안여부);
	 	  								==> 값 : true(적용), false(미적용, 기본값)
	 	  								
	 	  				==> 설정 값을 확인하려면 get으로 시작하는 해당 메서드를 호출하면 된다.
