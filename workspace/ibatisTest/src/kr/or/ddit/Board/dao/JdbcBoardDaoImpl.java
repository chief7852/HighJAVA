...
public class JdbcBoardDaoImpl implements IJdbcBoardDao {
	...
	public int updateBoard(JdbcBoardVO jBoardVo) {
		...
		try {
			...
			cnt = smc.delete("board.updateBoard",jBoardVo);
	...
	public List<JdbcBoardVO> getSearchBoardList(String jBoardTitle) {
		...
		try {
			...
			boardList = (ArrayList<JdbcBoardVO>)smc.queryForList("board.searchBoard", jBoardTitle);
	...
	public int setCountIncrement(int boardNo) {
		int cnt = 0;
		try {
			cnt = smc.update("board.increment",boardNo);
