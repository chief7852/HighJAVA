...
public class TcpMultiChatServer {
	...
	//clientMap에 저장된 모든 사용자에게 메세지 전송하는 메서드
		...
		public void run() {
			...
			try {
				while(true) {
					...
					// 이이름이 중복 되는지 여부를 feedback으로 
					//클라이언트에게 본낸다.
					name = dis.readUTF();	//클라이언트가 보낸 이름받기

					if(clientMap.containsKey(name)) {//이름 이 중복될때
						dos.writeUTF("이름 중복");
					}else {
						//중복되지않을때
						dos.writeUTF("OK");
				...
				System.out.println("현재 서ㅏ버 접속자 수 : " + clientMap.size() + "명");
