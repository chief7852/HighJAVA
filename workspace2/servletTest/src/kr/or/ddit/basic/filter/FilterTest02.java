...
1. 사용목적
	...
	- 클라이언트에 대한 응답 정보에 필요한 작업을 수행할 수 있다.
2. 사용예
	- 인증 작업용 필터
	- 데이터 압축용 필터
	- 인코딩용 필터
...
public class FilterTest02 implements Filter {
	...
	// 순서 init => doFilter => destroy
	
	// 필터 초기화 부분 ==> 웹서버가 실행될 때 필터드을 한번만 생성하는데
	// 				그 때 자동으로 호출되는 메서드 (주로 초기화 작업을 수행한다.)
	@Override
	public void init(FilterConfig config) throws ServletException {
		// init()메서드 필터의 초기화 파라미터를 읽어올 수 있다.
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		chain.doFilter(request, response); 		//다음 Filter로 체이닝하는 메서드(중요 기준점)
