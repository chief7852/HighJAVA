...
/*
 ...
 3. 질의 ==> SQL문을 작성해서 DB서버로 보내고 그 결과를 얻어온다.
 			(Statement객체나 PrepardeStatement객체를 이용하여 작업한다.)

 4. 결과를 처리 ==> 질의 결과를 받아서 원하는 작업을 수행한다.
 		1) SQL문이 select문일 경우에는 select한 결과가 ResultSet객체에
 		   저장되어 반환된다.
 		2) SQL문이 select문이 아닌경우(insert, update, delete, create 등...)
 		   에는 정수값이 반환된다. ( 이 정수값은 보통 해당 명령이 실행에 성공한 레코드 수가 된다.)
 ...
 */
	public static void main(String[] args) {
		...
		try {
			...
			// 4. 결과 처리하기 ==> 한 레코드씩 화면에 출(력하기
			System.out.println(" == 쿼리문 처리 결과 ==");
			
			// ResultSet객체에 저장된 데이터를 차례로 꺼내오려면
			// 반복문과 next()메서드를 이용한다.
			// rs.next(0 ==> ResultSet객체의 데이터를 가리키는
			//				포인터를 다음번째의 레코드로 이동시키고 그곳에
			// 				데이터가있으면 true를 반환한다.
			while(rs.next()){
				// 포인터가 가리키는 곳의 데이터를 가져오는 방법
				// 1) rs.get자료형 이름("컬럼명")
				// 2) rs.get자료형 이름(컬럼번호) ==> 컬럼번호는 1번부터시작
