...
/*
 	...
 	- Comparable은 Collection에 추가되는 데이터 자체에 정렬기준을
 	   넣고 싶을 때(내부정렬 기준) 구현하는 interface이고,
 	- Comparator는 외부에 별도로 정렬 기준을 구현하고 싶을 때 구현(외부정렬 기준)하는 interface이다.
...
public class ListSortTest1 
{
	...
	public static void main(String[] args) 
	{
		...
		// 			==> 내부정렬 기준이 구현되어 있는 데이터일 경우에 사용가능
		// 형식2) Collections.sort(리스트, 외부정렬기준 객체의 인스턴스);
		//			==> 외부정렬 기준에 맞게 정렬한다.
		Collections.sort(list);					//자동으로 오름차순 정렬
		System.out.println("정렬 후 : " + list);
		
		Collections.shuffle(list);	//데이터섞기
...
// 정렬 방식을 외부에서 정해 주는 class작성하기(지금은 내림차순으로)
...
class Desc implements Comparator<String>
{
	...
	// 반환값이 0이면 두 값이 같다.
	// 반환값이 양수이면 앞, 뒤의 순서를 바꾼다.(str2가 더 크다는뜻)
	// 반환값이 음수이면 앞, 뒤의 순서를 바꾸지 않는다.(str1이 크다는 뜻)
	
	// 예) 오름차순일 경우 ==> 앞의 값이 크면 양수, 같으면 -
