...
public class TcpServer01 {
	...
	public static void main(String[] args) throws IOException {
		// TCP소켓 통신을 위한SercerSocket객체를 생성한다.
		ServerSocket server = new ServerSocket(7777);
		System.out.println("서버가 접속을 기다립니다.");
		
		// accept()메서드
		//  ==> 클라이언트에서 연결 요청이 올 때까지 계속 기다린다.
		//  ==> 연결요청이 오면 Socket객체를 생성해서 클라이언트의
		//		Socket과 연결한다.
		Socket socket = server.accept();	//이부분을 accept()에 의해서 클라이
											//언트와 연결된 후에 처리할 내용을
											//기술한다.
		...
		// 클라이언트에게 메세지 전소하기
		// 위에서 구한OutputStream 객체의 write명령을 사용하면 된다.
