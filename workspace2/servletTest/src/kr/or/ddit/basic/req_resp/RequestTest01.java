...
public class RequestTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// getParameter("파라미터명"); ==> 해당 파라미터에 설정된 값을가져온다(태그에 name으로 지정한값)
		//					==> 가져온 '값'의 자료형은 무조건 'String'이다
		String userName= request.getParameter("username");
		String job = request.getParameter("job");
		
		// getParameterValues("파라미터명");
		// 		==> 파라미터명이 같은 값이 여러개일 경우에 사용한다.
...
//		}
		...
		//------------------------------------------------------------
//		getParameterMap() == >전송된 모든 파라미러를 Map객체에 담아서 반환한다.
		//		이Map객체의 key값은 '파라미터명'이며 자료형은 'String'이고
